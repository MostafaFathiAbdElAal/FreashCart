...
export default function DatailsBrand({ dataSpecificBrand, setDataSpecificBrand }) {
    ...
    return <>
        ...
        <div className={`w-3/4 sm:max-md:w-[60%] md:w-1/2 mt-32 bg-white rounded-md p-4 space-y-5 ${isShow ? "" : "-translate-y-full opacity-0"} ${isClose ? "-translate-y-full opacity-0" : ""} transition-[transform,opacity,left] duration-[700ms,500ms]`}>
            <header className="flex justify-end text-[#8e8e8e]">
            ...
            }} className="btn-success btnCloseBrand ms-auto block text-base font-semibold bg-[#6c757d] px-3 hover:bg-[#5c636a]">
                Close
            </button>
        </div>
    </>
